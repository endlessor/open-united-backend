    ...
    def handle(self, *args, **options):
        ...
        for task in tasks.iterator():
            tasklisting = TaskListing.objects.create(
                ...
                status=task.status,
                stacks=list(task.stack.all().values_list('name', flat=True)),
                ...
                product_data={
                    ...
                    "owner": task.product.get_product_owner().username
                ...
                initiative_data=model_to_dict(task.initiative) if task.initiative else None,
                capability=task.capability,
                capability_data=model_to_dict(task.capability) if task.capability else None,
            )
            task_claim = task.taskclaim_set.filter(kind__in=[0, 1]).first()
            if task_claim:
                tasklisting.task_claim = model_to_dict(task_claim)
                tasklisting.assigned_to_data = get_person_data(task_claim.person)
                tasklisting.assigned_to = task_claim.person
            tasklisting.save()
