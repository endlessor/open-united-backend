...
def sign_up(person_input, slug_increment):
    ...
    is_exists_person_by_username = Person.objects.filter(username=available_slug).exists()
    is_exists_user_by_slug = User.objects.filter(username=available_slug).exists()
    is_exist_username_by_slug = Username.objects.filter(username=available_slug).exists()

    user_email = person_input["email"]

    if is_exists_person_by_username or is_exists_user_by_slug or is_exist_username_by_slug:
        ...
        full_name = person_input.get("name", "")
        if not full_name:
            full_name = available_slug
...
def update_user(person_input, user):
    ...
    try:
        ...
        uname = user.username
        slug_increment = 0
        while Username.objects.filter(username=uname).exists():
            slug_increment += 1
